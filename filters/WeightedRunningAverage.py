...
class WeightedRunningAverage(QtWidgets.QDialog, Ui_RunningAverageDialog):
    def __init__(self, datastore):
        QtWidgets.QDialog.__init__(self)
        self.setupUi(self)
        self.setWindowTitle('Weighted Running Average')
    ...
    def average(self):
        ...
        for x in range(number_of_cols):
            ...
            if type(preprocessed_data_columns[x][0]) is datetime.datetime:
                ...
                processed_data_columns.append([np.average(preprocessed_data_columns[x][i:i + average_over_n_samples],
                                               weights=data_weights)
                                               for i in range(0, len(preprocessed_data_columns[x]), 1)
                                               if i + average_over_n_samples <= len(preprocessed_data_columns[x])])
